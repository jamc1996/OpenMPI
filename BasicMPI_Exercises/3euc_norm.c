...
/* 
  3euc_norm.c -- program to calculate sum of squares and 
                 Euclidean norm of vector from input file.

  Author: John Cormican

  Purpouse: Use multiple processors to calculate Euclidean Norm
...
int main(int argc, char *argv[])
{
  ...
  if(myid == root)
  {
    ...
    // Freeing malloc-ed data on root processor:
    free(starts);
    free(vector);
  }
  free(part);
...
Updates length to take length of vector and returns the array itself. */
{
  ...
  *length = atoi(buffer);
  ...
  //Buffer freed, file closed and array returned:
  free(buffer);
  fclose(fp);
  return array;
}
